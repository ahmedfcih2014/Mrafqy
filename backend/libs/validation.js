...
Validator.registerAsync('uniqueExceptId', function(value,  attribute, req, passes) {
    if (!attribute) throw new Error('Specify Requirements i.e fieldName: unique:table,column,exceptedId')
    //split table and column
    let attArr = attribute.split(",")
    if (attArr.length !== 3) throw new Error(`Invalid format for validation rule on ${attribute}`)

    //assign array index 0 and 1 to table and column respectively
    const { 0: table, 1: column, 2: exceptedId } = attArr
    ...
    // i have injected data direct in query ,because this query will used by code for validation
    // which means in case its have a sql injection will not be public use
    pgPool.query(`SELECT * FROM ${table} WHERE ${column} = '${value}' and id != '${exceptedId}'`)
    .then(result => {
        if (result.rowCount > 0) {
            passes(false, msg)
            return
